...
Процедура ЗагрузитьЖурналНаСервере()
	...
	Если ОчиститьПерезЗагрузкой Тогда
		ОбновлениеДанных.ЗагрузкаЖурналаПредварительноОчистить(Замер,КаталогТЖ);
	Иначе
		ОбновлениеДанных.ЗагрузкаЖурнала(Замер,КаталогТЖ);
	КонецЕсли;
...
Процедура ЗапуститьЗагрузкуНаСервере()
	...
	Иначе
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбновлениеДанных.ЗагрузкаЖурнала",мПараметры);
	КонецЕсли;
