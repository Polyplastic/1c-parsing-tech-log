...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	Иначе
		...
		Если Найти(Параметры.ИмяПользователя, """") Тогда
			Параметры.ИмяПользователя = СтрЗаменить(Параметры.ИмяПользователя, """", """""");
		КонецЕсли;
		
		Если Найти(Параметры.ПарольПользователя, """") Тогда
...
//	ЭтоВебКлиентПодMacOS = СтандартныеПодсистемыСервер.ПараметрыКлиентаНаСервере(
//		).Получить("ЭтоВебКлиентПодMacOS");
	ЭтоВебКлиентПодMacOS = Неопределено;
...
Функция ПроверкаВалидностиЭлемента(ИмяЭлемента) Экспорт
	Результат = Истина;
	
	Попытка
		Структура = новый Структура(ИмяЭлемента);
	Исключение
		Результат = Ложь;
	КонецПопытки;
