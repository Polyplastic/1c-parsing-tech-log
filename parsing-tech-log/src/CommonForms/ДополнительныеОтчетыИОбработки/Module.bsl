...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок[Параметры.Вид];
	
	Если ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет Тогда
...
Процедура НастроитьСписок(Команда)
	//TODO: Додельать настройку под пользователя
...
Процедура ВыполнитьОбработкуПоПараметрам()
	...
	ВыполняемаяКоманда = Новый Структура(
		...
		|Идентификатор, ВариантЗапуска, ПоказыватьОповещение, ИмяФормы,
...
Функция ПроверитьВыполнениеФоновогоЗаданияНаСервере(ФоновоеЗаданиеИдентификатор, ФоновоеЗаданиеАдресХранилища, Отменить = Ложь)
	...
	//		Задание.Завершено = Истина;
	//		Задание.Успешно   = Истина;
	//		Задание.Значение  = ПолучитьИзВременногоХранилища(ФоновоеЗаданиеАдресХранилища);
	//	КонецЕсли;
	//Исключение
	//	Задание.Завершено = Истина;
	//	Задание.Успешно   = Ложь;
	...
	//	СтандартныеПодсистемыКлиентСервер.ВывестиИнформациюОбОшибке(Задание.Значение, Текст, ИнформацияОбОшибке());
	//КонецПопытки;
	//Если Не Задание.Завершено И Отменить Тогда
...
Процедура ТаблицаКомандПриАктивизацииСтроки(Элемент)
	...
	HtmlОписание = СформироватьОписаниеКоманды(ТекущиеДанные.Представление,ТекущиеДанные.Описание,ТекущиеДанные.Версия);
КонецПроцедуры
...
Функция СформироватьОписаниеКоманды(Знач Представление,Знач Информация,Знач Версия)
	...
	Информация = "<div>"+СтрЗаменить(Информация,Символы.ПС,"</div><div>")+"</div>";
	Информация = СтрЗаменить(Информация,"<div></div>","");
	html = html +"<h3>"+Представление+"</h3>";
	html = html +"<div style='text-align:right'>Версия: "+Версия+"</div><br />";
