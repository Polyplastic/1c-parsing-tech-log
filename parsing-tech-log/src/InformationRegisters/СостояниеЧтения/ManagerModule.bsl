Функция ПолучитьСостояние(ЗНАЧ ФайлЗамера) Экспорт
	Результат = Новый Структура("ПрочитаноСтрок,ЧтениеЗавершено,Размер,ДатаИзмененияСостояния,ДатаПрочитанныхДанных,Ошибка,ОписаниеОшибки,СобытиеЗамера",
		0, Ложь, 0, Дата(1,1,1), Дата(1,1,1), Ложь, "",ПредопределенноеЗначение("Справочник.КлючиСобытия.ПустаяСсылка"));
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СостояниеЧтения.СобытиеЗамера КАК СобытиеЗамера
	               |ИЗ
...
Процедура УстановитьСостояние(
		...
		СобытиеЗамера=Неопределено) Экспорт
	...
	МенеджерЗаписи.СобытиеЗамера 			= СобытиеЗамера;
