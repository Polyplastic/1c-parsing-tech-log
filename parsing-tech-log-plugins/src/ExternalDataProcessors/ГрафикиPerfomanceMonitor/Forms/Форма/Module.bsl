...
#Область НастройкиЗагрузитьСохранить

&НаКлиенте
Процедура ЗагрузитьИзОбщихНастроек(Команда)
	
	мНастройка = УправлениеХранилищемНастроекВызовСервера.ДанныеИзБезопасногоХранилища("ГрафикиPerfomanceMonitor.ОбщаяНастройка");
	Если мНастройка<>Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		Сообщить("В общих настройках нет сохраненного шаблона!");
	КонецЕсли;
...
Процедура СохранитьВОбщиеНастройки(Команда)
	...
	мНастройка.Вставить("ТипДиаграммыДанныхИзБазы",ТипДиаграммыДанныхИзБазы);
	мНастройка.Вставить("ГруппировкаПоВремени",ГруппировкаПоВремени);
	мНастройка.Вставить("ГотовыеВариантыИнтервалИзБазы",ГотовыеВариантыИнтервалИзБазы);
	мНастройка.Вставить("ИнтервалИзБазы",ИнтервалИзБазы);
	мМассивСтруктур = новый Массив;
	Для каждого стр из ТаблицаИсточниковИзБазы Цикл
		стр_н = Новый Структура("Выбрана,Замер,Свойство,Представление,Смещение,ТипСобытияКлюч");
		ЗаполнитьЗначенияСвойств(стр_н,стр);
		мМассивСтруктур.Добавить(стр_н);
	КонецЦикла;
	мНастройка.Вставить("ТаблицаИсточниковИзБазы",мМассивСтруктур);
	УправлениеХранилищемНастроекВызовСервера.ЗаписатьДанныеВБезопасноеХранилищеРасширенный("ГрафикиPerfomanceMonitor.ОбщаяНастройка",мНастройка,"Общая настройка для графика замеров");
