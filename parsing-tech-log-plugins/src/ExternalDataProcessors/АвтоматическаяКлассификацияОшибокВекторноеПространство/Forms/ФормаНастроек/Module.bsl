...
Процедура ПриОткрытии(Отказ)
	...
	РежимРаботыРедактораПриИзменении(Неопределено);
КонецПроцедуры

&НаКлиенте
Процедура РежимРаботыРедактораПриИзменении(Элемент)
	Элементы.Страницы.Видимость = (РежимРаботыРедактора="Расширенный");
	Элементы.СтраницыУпрощеннаяНастройка.Видимость = (РежимРаботыРедактора="Упрощенный");
КонецПроцедуры
...
Процедура ЗагрузитьНастройки(Команда)
	...
	Если мНастройка<>Неопределено Тогда
		...
		ВекторРассчитан=(БазисСлов.Количество()>0);
...
&НаСервере
Процедура ПолучитьНастройкиПоУмолчанию()
	
	// общие
	РазмерПакета = 1000;
	ЗаписыватьРезультатОбработкиВИсходныйЗамер = Истина;
	ЗаписыватьРезультатОбработкиТекущийЗамер = Истина;
	ГруппировкаПоВремени = "10_мин";
	
	// обработка текста
	Текст_ИспользоватьСтеммерПортера = Истина;
	Текст_РазделятьБольшойМаленький = Истина;
	Текст_ЗаменитьСинонимы = Истина;
	Текст_УдалятьЦифры = Истина;
	Текст_УдалятьСтопФразы = Истина;
	Текст_УдалятьСтопСлова = Истина;
	Текст_УдалятьCPP = Истина;
	Текст_УдалятьServerAddr = Истина;
	Текст_УдалятьURL = Истина;
	Текст_УдалятьОшибкаКРесурсу = Истина;
	Текст_УдалятьНомерДатаДокумента = Истина;
	Текст_УдалятьПараметрыSQLЗапросов = Истина;
	Текст_УдалятьСтекВызова = Ложь;
	Текст_УдалятьКомментарииКода = Истина;
	
	
	МинимальнаяЧастотаПроцент = 0;
	МаксимальнаяЧастотаПроцент  = 100;
	
	// дополнительно
	ЗаписыватьРезультатОбработкиВКлассификатор = Ложь;
	СохранятьВторойКласс = Ложь;
...
&НаКлиенте
Процедура ВыполитьРасчетВекторногоПространства(Команда)
	
	РезультатАвтоматическогоРасчета = "";
	
	// 1 Классификатор
	ВыбратьДанныеИзКлассификатора(Неопределено);
	РезультатАвтоматическогоРасчета = РезультатАвтоматическогоРасчета+"1. Данные выбраны из классификатора"+Символы.ПС;
	// 2 Обработка текста
	ОбработатьТекст(Неопределено);                                       
	РезультатАвтоматическогоРасчета = РезультатАвтоматическогоРасчета+"2.1. Текст обработан"+Символы.ПС;
	ЗаполнитьТаблицуКорпусаТекстовПоТекущемуТексту(Неопределено);                                  
	РезультатАвтоматическогоРасчета = РезультатАвтоматическогоРасчета+"2.2. Корпус текстов заполнен"+Символы.ПС;
	// 3 Базис
	ПолучитьЧастотность(Неопределено);                                                                     
	РезультатАвтоматическогоРасчета = РезультатАвтоматическогоРасчета+"3.1. Получили частотность"+Символы.ПС;
	УдалитьРедкиеЧастыеСлова(Неопределено);	                                                            
	РезультатАвтоматическогоРасчета = РезультатАвтоматическогоРасчета+"3.2. Удалены частые и редки слова"+Символы.ПС;
	СформироватьТаблицуВекторов(Неопределено);                                                                  
	РезультатАвтоматическогоРасчета = РезультатАвтоматическогоРасчета+"3.3. Сформировали таблицу векторов"+Символы.ПС;
	РассчитатьВекторноеПространство(Неопределено);                                                               
	РезультатАвтоматическогоРасчета = РезультатАвтоматическогоРасчета+"3.4. Выполнен расчет векторного классификатора"+Символы.ПС;
	РезультатАвтоматическогоРасчета = РезультатАвтоматическогоРасчета+Символы.ПС+"Автоматический расчет выполнен"+Символы.ПС;
