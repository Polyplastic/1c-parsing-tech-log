...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия", "2019.11.09");
...
Процедура ДобавитьКоманду(ТаблицаКоманд, Представление, Идентификатор, Использование, ПоказыватьОповещение = Ложь, Модификатор = "", ИмяФормы="")
...
Функция ЗагрузитьДанныеВЗамерСервер(Замер) Экспорт
	...
	СоответствиеСвойств = ПолучитьСоответсвиеСинонимовСвойств();	
	...
	Для каждого стр из СоответсвиеКластеровКорзины Цикл
		...
		Для каждого эл_спис из стр.Значение Цикл
			...
			Если СохранятьВсеДетальныеЗаписи=Истина Тогда
				...
				Для каждого стр_замер из СтруктураДанныхОтвета.МассивСоответствиеДанных Цикл
					...
					Для каждого эл_св из стр_замер Цикл
						Свойство = ПолучитьСвойствоБезопасно(СоответствиеСвойств.Получить(эл_св.Ключ),эл_св.Ключ);
			...
			Для каждого эл_агрег из МассивСтруктурАгрегацииДанных Цикл
				Свойство = ПолучитьСвойствоБезопасно(СоответствиеСвойств.Получить(эл_агрег.name),эл_агрег.name,эл_агрег.func);
...
Процедура RAS_СоздатьСвойстваВБазеЕслиНужно() Экспорт
	...
	Для каждого стр из  СоответствиеСвойств Цикл
		...
		СвойствоОбъект.Наименование = стр.Значение;
		СвойствоОбъект.Код = стр.Ключ;
		СвойствоОбъект.Синоним = стр.Ключ;
		СвойствоОбъект.Код = стр.Ключ;
		
		Если Найти(СвойствоОбъект.Синоним,"time")
			ИЛИ СвойствоОбъект.Синоним="line-number" 
			ИЛИ СвойствоОбъект.Синоним="db-proc-took" 
			ИЛИ СвойствоОбъект.Синоним="connections" 
			ИЛИ Найти(СвойствоОбъект.Синоним,"duration") 
			ИЛИ Найти(СвойствоОбъект.Синоним,"write") 
			ИЛИ Найти(СвойствоОбъект.Синоним,"read") 
			ИЛИ Найти(СвойствоОбъект.Синоним,"-5min") 
			ИЛИ Найти(СвойствоОбъект.Синоним,"-total") 
			ИЛИ Найти(СвойствоОбъект.Синоним,"-current") 
			ИЛИ Найти(СвойствоОбъект.Синоним,"-all") 
			ИЛИ Найти(СвойствоОбъект.Синоним,"-size") 
			ИЛИ Найти(СвойствоОбъект.Синоним,"-perfomance") 
