...
Процедура ВычислитьФункцииАгрегации(Знач МассивСоответствиеДанных,Знач list)
	...
	Для каждого стр из МассивСтруктурАгрегацииДанных Цикл
		...
		ЗаполнитьЗначенияСвойств(стр_н,стр);
		УстановитьУровеньПроблемыСтроки(стр_н);
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьУровеньПроблемыСтроки(стр)
	
	Если стр.name="available-perfomance"
		И стр.func="min" Тогда
			
		Если стр.value<50 Тогда
			стр.decision="красный";
		ИначеЕсли стр.value<100 Тогда
			стр.decision="желтый";
		КонецЕсли;
		
	ИначеЕсли стр.name="duration-current" 
		И стр.func="max" Тогда
...
Процедура СоздатьИОбновитьКолонки(Знач МассивСоответствиеДанных,Знач list)
	...
	Если СоставКолонокОдинаковый(Колонки,СтарыеКолонки)=Истина Тогда
		Возврат;
	КонецЕсли;
