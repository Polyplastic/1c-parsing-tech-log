...
Процедура ОбновитьДанныеФоново(ТекущиеДанные) Экспорт
	...
	СведенияФЗ = ЗапуститьЗадание24(АдресХранилища,Список,мПараметры,новый УникальныйИдентификатор());
...
&НаСервереБезКонтекста
//&НаСервере
Функция ЗапуститьЗадание24(АдресХранилища,list,мПараметры,УникальныйИдентификатор)
	...
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ДополнительныеОтчетыИОбработки.ВыполнитьКоманду", мМассив);
...
Процедура УстановитьУровеньПроблемыСтроки(стр,ЭтоТаблица=Истина)
	...
	Если стр.name="available-perfomance"
		И (ЭтоТаблица=Истина ИЛИ стр.func="min") Тогда
			
		Если стр.value<50 Тогда
			decision="красный";
		ИначеЕсли стр.value<100 Тогда
			decision="желтый";
		КонецЕсли;
		
	ИначеЕсли стр.name="duration-current" 
		И (ЭтоТаблица=Истина ИЛИ стр.func="max") Тогда
			
		Если стр.value>300000 Тогда // 5 мин
			decision="красный";
		ИначеЕсли стр.value>60000 Тогда // 1 мин
			decision="желтый";
		КонецЕсли;
		
	ИначеЕсли стр.name="duration-current" 
		И стр.func="count ЗначениеЗаполнено" Тогда
			
		Если стр.value>40 Тогда // 5 мин
			decision="красный";
		ИначеЕсли стр.value>20 Тогда // 1 мин
			decision="желтый";
		КонецЕсли;	
			
	ИначеЕсли стр.name="db-proc-took" 
		И (ЭтоТаблица=Истина ИЛИ стр.func="max") Тогда
	...
	ИначеЕсли стр.decision="красный" И decision="желтый" Тогда
		стр.decision = decision;
	КонецЕсли;
...
Процедура СоздатьИОбновитьКолонкиПустыеКолонки()
	...
	"empty","empty","Строка",ОписаниеТипов,10,"путо (обновите данные)","empty",Неопределено,Истина,Истина));
...
Процедура ОбновитьГрафикИстории()
	...
	ОбновитьГрафикИсторииСервер(Список,ТекущиеДанные.name,ТекущиеДанные.synonim,ТекущиеДанные.func);
...
&НаСервере
Процедура ОбновитьГрафикИсторииСервер(ТипСобытия,name,synonim,func)
	...
	|	И СобытияЗамераКлючевыеСвойства.Ссылка.НомерСтрокиФайла = 0
	|	И СобытияЗамераКлючевыеСвойства.Ссылка.ТипСобытия.Наименование = &ТипСобытия
