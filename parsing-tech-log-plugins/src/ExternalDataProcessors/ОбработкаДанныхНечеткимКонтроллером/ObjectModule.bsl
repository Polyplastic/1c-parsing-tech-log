...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия", "2019.10.17");
	...
	Пока ЕстьДанныеДляОбработки=Истина Цикл
		...
		Для каждого стр из ТаблицаДанных Цикл
			...
			Если СтрукутраДанных = Неопределено Тогда
				...
				СтрукутраДанных.input.Добавить(новый структура("СвойствоНаименование,НомерВхода,Значение,ЗначениеЧисло",стр.СвойствоНаименование,стр.НомерВхода,стр.Значение,стр.ЗначениеЧисло));
		...
		Для каждого дата_стр из СоответствиеДат Цикл
			...
			Для каждого эл из input Цикл
				терм = DoFuzzification(FuzzyModel,FuzzyModel.Fuzzification[эл.НомерВхода].name,эл.Значение,эл.ЗначениеЧисло);
...
Функция DoFuzzification(FuzzyLogic,Name,ValueString,Value)
...
Функция DoDecision(FuzzyLogic,Inputs)
	...
	Для каждого rule из FuzzyLogic.FuzzyController.Rules Цикл
		...
		Для каждого элемент из rule Цикл
			...
			Если элемент.name="decision" И элемент.type="output" Тогда
				decision = элемент.value;
			КонецЕсли;
			
			Если элемент.name="description" И элемент.type="output"  Тогда
				description = элемент.value;
			КонецЕсли;
			
			Если элемент.type="output" Тогда
				Продолжить;
			КонецЕсли;
			...
			Если НЕ нрег(СоответсвиеВходов.Получить(элемент.name))=нрег(элемент.value) Тогда
 					...
 					Пока ЧтениеXML.Прочитать() Цикл
						...
						ИначеЕсли  ЧтениеXML.Имя="rules" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента  Тогда
							...
							Пока ЧтениеXML.Прочитать() Цикл
								...
								ИначеЕсли ЧтениеXML.Имя = "input" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
									Структура = новый Структура();
									Структура.Вставить("type","input");
									Структура.Вставить("name",ЧтениеXML.ПолучитьАтрибут("name"));
									Структура.Вставить("value",ЧтениеXML.ПолучитьАтрибут("value"));
									Rule.Добавить(Структура);
								ИначеЕсли ЧтениеXML.Имя = "decision" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
									Структура = новый Структура();
									Структура.Вставить("type","output");
									Структура.Вставить("name","decision");
									Структура.Вставить("value",ЧтениеXML.ПолучитьАтрибут("value"));
									Rule.Добавить(Структура);
								ИначеЕсли ЧтениеXML.Имя = "priority" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
									Структура = новый Структура();
									Структура.Вставить("type","output");
									Структура.Вставить("name","priority");
									ЧтениеXML.Прочитать();
									Структура.Вставить("value",Число(ЧтениеXML.Значение));
									Rule.Добавить(Структура);
								ИначеЕсли ЧтениеXML.Имя = "description" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
									Структура = новый Структура();
									Структура.Вставить("type","output");
